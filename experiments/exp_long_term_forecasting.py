...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        if self.args.fm:
            ...
            self.model.load_state_dict(torch.load(os.path.join('./checkpoints/' + "fm", 'checkpoint.pth')))
        path = os.path.join(self.args.checkpoints, setting)
        if not os.path.exists(path):
            os.makedirs(path)
        if self.args.fm:
            path = os.path.join('./checkpoints',"fm")
        time_now = time.time()
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader_iter):
                #print(f"now_time_cost_1:{time.time() - time_cur}") 
                iter_count += 1
                ...
                if self.args.use_amp:
                    ...
                    loss = criterion(outputs, batch_y)
                    train_loss.append(loss.item())
                #print(f"now_time_cost_2:{time.time() - time_cur}") 
                if (i + 1) % 100 == 0:
